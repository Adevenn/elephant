...
class Controller implements InteractionToController{
  ...
  Future<void> addCheckbox(int idParent) async{
    try{
      var json = jsonEncode(Checkbox(id: -1, idParent: idParent, text: '', idOrder: -1));
  ...
  Future<void> addImage(int idParent, Uint8List previewImage, Uint8List rawImage) async{
    try{
      var json = jsonEncode(Image.full(id: -1, imgPreview: previewImage, imgRaw: rawImage, idParent: idParent, idOrder: -1));
  ...
  Future<void> addTexts(int idParent, int txtType) async{
    try{
      var json = jsonEncode(Text(text:'', idParent: idParent, txtType: TextType.values[txtType], id: -1, idOrder: -1));
